...
(function () {
  ...
  function getCart(){
    try {
      ...
      c.forEach(function (i){ if (i && i.photo && /kaysun-prodigy-indoor\.png/.test(i.photo)) i.photo = "assets/products/kaysun-casual-indoor.webp"; });
      return c;
    } catch (e) { return []; }
  }
  ...
  document.addEventListener("click", function (e) {
    ...
    addToCart(item);
    showToast("Додано в заявку: " + item.name);
  });
  ...
  function rowHTML(i){
    ...
    return '<div class="cart-row" data-id="' + esc(i.id) + '">'
      + '<div class="cart-row__media"><div class="ph">' + (i.photo ? '<img class="prod-photo" src="' + esc(i.photo) + '" alt="' + esc(i.name) + '">' : '<span>' + esc(i.img) + '</span>') + '</div></div>'
      + '<div class="cart-row__info"><b>' + esc(i.name) + '</b>'
        + (i.ver ? '<span class="cart-row__ver">Версія: ' + esc(i.ver) + '</span>' : '')
        + (i.spec ? '<span class="cart-row__spec">' + esc(i.spec) + '</span>' : '')
        + '<button class="cart-row__rm-text" data-act="remove">Видалити</button></div>'
      + '<div class="qty" role="group" aria-label="Кількість">'
        + '<button data-act="dec" aria-label="Зменшити">\u2212</button>'
        + '<span class="qty__n">' + i.qty + '</span>'
        + '<button data-act="inc" aria-label="Збільшити">+</button></div>'
      + '<div class="cart-row__price"><span class="price__main price__uah">' + lineU + ' грн</span>'
        + '<span class="price__eur">' + lineE + ' €</span></div>'
      + '<button class="cart-row__rm" data-act="remove" aria-label="Видалити товар">'
  ...
  function renderCart(){
    ...
    setText("#sum-count", qn + " " + plural(qn, "товар", "товари", "товарів"));
    setText("#cart-count-label", qn + " " + plural(qn, "товар", "товари", "товарів"));
  }
  ...
  if (cartForm) {
    ...
    if (cartUser){
      ...
      note.innerHTML = '<svg viewBox="0 0 24 24" width="18" fill="none" stroke="#6E54A6" stroke-width="2"><circle cx="12" cy="8" r="4"/><path d="M5 21v-2a5 5 0 0 1 5-5h4a5 5 0 0 1 5 5v2"/></svg><span>Заявка від <b>' + esc(byName || cartUser.email) + '</b>' + (cartUser.email ? '<br>· ' + esc(cartUser.email) : "") + (cartUser.phone ? '<br>· ' + esc(cartUser.phone) : "") + '</span>';
  ...
  var PRODUCTS = [
      ...
      { id:"EN-AC-KAYSUN-CASUAL-26", name:"Kaysun Casual AKAY-C 26", brand:"Kaysun", series:"casual", type:"wall", sub:"split", btu:9, area:25, power:2.6, range:"2.6", uah:20250, eur:450, energy:"A++", wifi:1, minTemp:-15, badge:"new", pop:95, rating:4.8, warranty:3, stock:"in", eta:0, href:"kaysun-casual.html", img:"настінний блок", photo:"assets/products/kaysun-casual-indoor.webp" },
      { id:"EN-AC-KAYSUN-CASUAL-35", name:"Kaysun Casual AKAY-C 35", brand:"Kaysun", series:"casual", type:"wall", sub:"split", btu:12, area:35, power:3.5, range:"3.5", uah:23850, eur:530, energy:"A++", wifi:1, minTemp:-15, badge:"", pop:88, rating:4.7, warranty:3, stock:"in", eta:0, href:"kaysun-casual.html", img:"настінний блок", photo:"assets/products/kaysun-casual-indoor.webp" },
      { id:"EN-AC-KAYSUN-CASUAL-52", name:"Kaysun Casual AKAY-C 52", brand:"Kaysun", series:"casual", type:"wall", sub:"split", btu:18, area:50, power:5.28, range:"5.28", uah:29250, eur:650, energy:"A++", wifi:1, minTemp:-15, badge:"", pop:80, rating:4.7, warranty:3, stock:"in", eta:0, href:"kaysun-casual.html", img:"настінний блок", photo:"assets/products/kaysun-casual-indoor.webp" },
      { id:"EN-AC-KAYSUN-CASUAL-71", name:"Kaysun Casual AKAY-C 71", brand:"Kaysun", series:"casual", type:"wall", sub:"split", btu:24, area:70, power:7.03, range:"7.03", uah:35550, eur:790, energy:"A++", wifi:1, minTemp:-15, badge:"", pop:78, rating:4.6, warranty:3, stock:"order", eta:0, href:"kaysun-casual.html", img:"настінний блок", photo:"assets/products/kaysun-casual-indoor.webp" },
      // Kaysun Prodigy PRO (R-32, A+++) — настінні спліт-системи, 4 типорозміри
      { id:"EN-AC-KAYSUN-PRODIGY-26", name:"Kaysun Prodigy PRO AKAY-P 26", brand:"Kaysun", series:"prodigy", type:"wall", sub:"split", btu:9, area:25, power:2.73, range:"2.73", uah:26550, eur:590, energy:"A+++", wifi:1, minTemp:-15, badge:"hit", pop:96, rating:4.9, warranty:3, stock:"in", eta:0, href:"kaysun-prodigy.html", img:"настінний блок", photo:"assets/products/kaysun-casual-indoor.webp" },
      { id:"EN-AC-KAYSUN-PRODIGY-35", name:"Kaysun Prodigy PRO AKAY-P 35", brand:"Kaysun", series:"prodigy", type:"wall", sub:"split", btu:12, area:35, power:3.52, range:"3.52", uah:30150, eur:670, energy:"A+++", wifi:1, minTemp:-15, badge:"", pop:90, rating:4.8, warranty:3, stock:"in", eta:0, href:"kaysun-prodigy.html", img:"настінний блок", photo:"assets/products/kaysun-casual-indoor.webp" },
      { id:"EN-AC-KAYSUN-PRODIGY-52", name:"Kaysun Prodigy PRO AKAY-P 52", brand:"Kaysun", series:"prodigy", type:"wall", sub:"split", btu:18, area:50, power:5.28, range:"5.28", uah:36450, eur:810, energy:"A+++", wifi:1, minTemp:-15, badge:"", pop:82, rating:4.8, warranty:3, stock:"in", eta:0, href:"kaysun-prodigy.html", img:"настінний блок", photo:"assets/products/kaysun-casual-indoor.webp" },
      { id:"EN-AC-KAYSUN-PRODIGY-71", name:"Kaysun Prodigy PRO AKAY-P 71", brand:"Kaysun", series:"prodigy", type:"wall", sub:"split", btu:24, area:70, power:7.04, range:"7.04", uah:43650, eur:970, energy:"A+++", wifi:1, minTemp:-15, badge:"", pop:80, rating:4.7, warranty:3, stock:"order", eta:0, href:"kaysun-prodigy.html", img:"настінний блок", photo:"assets/products/kaysun-casual-indoor.webp" },
      /* ТИМЧАСОВІ демо-картки (поки немає реальних товарів цих типів) — прибрати при наповненні каталогу */
      { id:"EN-DEMO-CONSOLE", name:"Консольний кондиціонер (демо)", brand:"Kaysun", series:"casual", type:"console", sub:"split", btu:12, area:35, power:3.5, range:"3.5", uah:27900, eur:620, energy:"A++", wifi:1, minTemp:-15, badge:"", pop:60, rating:0, warranty:3, stock:"order", eta:0, href:"#", img:"консольний блок", photo:"assets/types/console.webp" },
      { id:"EN-DEMO-DUCT", name:"Канальний кондиціонер (демо)", brand:"Kaysun", series:"casual", type:"duct", sub:"split", btu:24, area:70, power:7.0, range:"7.0", uah:54000, eur:1200, energy:"A+", wifi:0, minTemp:-15, badge:"", pop:58, rating:0, warranty:3, stock:"order", eta:0, href:"#", img:"канальний блок", photo:"assets/types/duct.webp" },
      { id:"EN-DEMO-CASSETTE", name:"Касетний кондиціонер (демо)", brand:"Kaysun", series:"casual", type:"cassette", sub:"split", btu:18, area:55, power:5.2, range:"5.2", uah:44100, eur:980, energy:"A++", wifi:0, minTemp:-15, badge:"", pop:56, rating:0, warranty:3, stock:"order", eta:0, href:"#", img:"касетний блок", photo:"assets/types/cassette.webp" },
      { id:"EN-DEMO-FLOORCEIL", name:"Підлогово-стельовий (демо)", brand:"Kaysun", series:"casual", type:"floorceil", sub:"split", btu:18, area:50, power:5.0, range:"5.0", uah:33300, eur:740, energy:"A++", wifi:1, minTemp:-15, badge:"", pop:54, rating:0, warranty:3, stock:"order", eta:0, href:"#", img:"підлогово-стельовий блок", photo:"assets/types/floorceil.webp" },
      { id:"EN-DEMO-MULTI-ODU", name:"Мульти-спліт · зовнішній блок (демо)", brand:"Kaysun", series:"casual", type:"multi", block:"outdoor", sub:"multi", btu:36, area:90, power:10.5, range:"10.5", uah:67500, eur:1500, energy:"A++", wifi:0, minTemp:-15, badge:"", pop:52, rating:0, warranty:3, stock:"order", eta:0, href:"#", img:"зовнішній блок", photo:"assets/types/outdoor.webp" },
      { id:"EN-DEMO-MULTI-IDU", name:"Мульти-спліт · внутрішній блок (демо)", brand:"Kaysun", series:"casual", type:"multi", block:"indoor", sub:"multi", btu:9, area:25, power:2.6, range:"2.6", uah:17100, eur:380, energy:"A++", wifi:1, minTemp:-15, badge:"", pop:50, rating:0, warranty:3, stock:"order", eta:0, href:"#", img:"внутрішній блок", photo:"assets/types/wall.webp" }
    ];
  ...
  function cardHTML(p){
      ...
      var brandBadge = BRAND_LOGO[p.brand]
        ? '<span class="spec-badge spec-badge--brand"><img src="assets/brands/' + BRAND_LOGO[p.brand] + '.svg" alt="' + p.brand + '"></span>'
      ...
      return '<article class="prod-card">'
        ...
        + '<button class="card-add" data-add-request data-id="' + p.id + '" data-name="' + p.name + '" data-spec="' + p.range + ' кВт · R-32 · ' + p.energy + '" data-uah="' + uahFromEur(p.eur) + '" data-eur="' + p.eur + '" data-img="' + p.img + '" data-photo="' + (p.photo || "") + '" aria-label="Додати в заявку"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M12 5v14M5 12h14"/></svg><span class="btn-label">Додати в заявку</span></button></div>'
        + '</div></div></article>';
    }
  ...
  if (catGrid) {
    ...
    function render(){
      ...
      refreshCardQty();
      setText("#catalog-count", list.length + " " + plural(list.length, "товар", "товари", "товарів"));
