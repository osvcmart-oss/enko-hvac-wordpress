...
(function () {
  ...
  (function injectAdminBtn(){ return; /* ENKO: admin link removed from header — access only via /manager/ */
  ...
  function renderQuickPop(force){
    ...
    if ((typeof force === "boolean") ? force : isWorking()){
      inner.innerHTML =
        '<div class="quick-pop__head"><span class="quick-pop__ava quick-pop__ava--online"><img class="qp-ava-logo" src="/wp-content/themes/enko/assets/logo-enko-white.png" alt="ENKO"></span><span><b>Михайло</b><span>Онлайн</span></span></div>'
      ...
      inner.innerHTML =
        '<div class="quick-pop__head"><span class="quick-pop__ava quick-pop__ava--offline"><img class="qp-ava-logo" src="/wp-content/themes/enko/assets/logo-enko-white.png" alt="ENKO"></span><span><b>Михайло</b><span>Зараз не на зв’язку</span></span></div>'
        + '<p>' + T("Зараз неробочий час.") + '<br>' + T("Відповідаємо: Пн–Пт 9:00–18:00") + '</p>'
  ...
  (function () {
    ...
    function build(force){
      ...
      var form;
      if (working){
        form = '<div class="callbar__txt"><b>' + T("Хочете, зателефонуємо Вам?") + '</b><span>' + T("Залиште номер — передзвонимо найближчим часом.") + '</span></div>'
          + '<form class="callbar__form" onsubmit="return false"><input type="tel" placeholder="' + T("Ваш номер телефону") + '" aria-label="' + T("Телефон") + '" required><button class="btn" type="submit">' + T("Чекаю на дзвінок") + '</button></form>';
      } else {
        ...
        // Компактний off-hours бар (вимога замовника): заголовок + підзаголовок ліворуч,
        // дата+час+телефон+кнопка праворуч в один рядок; рядок згоди — окремо під ними.
        form = '<div class="callbar__txt callbar__txt--sched"><b>' + T("На жаль, зараз у нас неробочий час.") + '</b><span>' + T("Запишіться на дзвінок — зателефонуємо Вам у зручний час:") + '</span></div>'
          + '<form class="callbar__form callbar__form--sched" onsubmit="return false"><input type="date" class="callbar__date" aria-label="' + T("Дата") + '" min="' + ymd(today) + '" max="' + ymd(maxD) + '" value="' + ymd(today) + '"><select class="callbar__time" aria-label="' + T("Час") + '">' + opts + '</select><input type="tel" placeholder="' + T("Ваш номер телефону") + '" aria-label="' + T("Телефон") + '" required><button class="btn" type="submit">' + T("Чекаю на дзвінок") + '</button></form>';
      }
    ...
    function buildCookie(force){
      ...
      bar.innerHTML = '<div class="container callbar__in cookiebar__in">'
        ...
        + '<span>Необхідні cookies — для роботи сайту, а також аналітичні та маркетингові — для покращення сервісу й персоналізації реклами. Докладніше — у <a href="/cookies/">Політиці щодо файлів cookie</a>.</span></div>'
