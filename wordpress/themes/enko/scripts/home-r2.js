...
(function () {
  ...
  function hrefForItem(id){
    ...
    if (window.ENKO_CART_LINKS && window.ENKO_CART_LINKS[id]) return window.ENKO_CART_LINKS[id]; // реальний permalink товару (WP)
    if (/KAYSUN-CASUAL/i.test(id)) return "kaysun-casual.html"; // фолбек прототипу
  ...
  function initCartDiscount(){
    ...
    function decorate(){
      ...
      if (d && t.u > 0){
        ...
        if (lbl) lbl.textContent = T("Разом зі знижкою");
        var box = document.createElement("div"); box.id = "cart-disc-lines"; box.className = "cart-disc-lines";
        box.innerHTML = '<div class="cart-disc-row"><span>' + T("Сума без знижки") + '</span><span class="price__old" style="margin:0">' + fmt(t.u) + ' грн</span></div>'
          + '<div class="cart-disc-row cart-disc-row--save"><span>' + T("Персональна знижка") + ' <b>−' + d + '%</b></span><span>−' + fmt(t.u - newU) + ' грн</span></div>';
  ...
  function initLangPills(){ return; /* ENKO i18n: серверний ?lang-перемикач; JS не чіпає */
  ...
  /* =====================================================================
     ...
     ===================================================================== */
  var HERO_SLIDES = [
    { img: "/wp-content/uploads/enko/catalog-conditioners.webp", alt: "Кондиціонери", cls: "hero__slide--cond" },
    { img: "/wp-content/uploads/enko/catalog-vrf.webp",          alt: "Мультизональні VRF", cls: "hero__slide--vrf" },
    { img: "/wp-content/uploads/enko/catalog-heat-pumps.webp",   alt: "Теплові насоси", cls: "hero__slide--hp" },
    { img: "/wp-content/uploads/enko/catalog-ventilation.webp",  alt: "Вентиляція", cls: "hero__slide--vent" },
    { img: "/wp-content/uploads/enko/catalog-microclimate.webp", alt: "Мікроклімат", cls: "hero__slide--micro" },
    { img: "/wp-content/uploads/enko/catalog-fancoils.webp",     alt: "Фанкойли", cls: "hero__slide--fan" }
  ];
  function initHeroSlider(){
    ...
    start();
    // Препідвантаження фонів УСІХ слайдів після load: CSS-фон качається лише в
    // момент показу слайда — без цього перемикання показує порожній градієнт
    // ~1.5с (покадровий розбір 2026-07-13). Вантажимо варіант під поточний екран.
    function warmSlides(){
      var mob = window.matchMedia && matchMedia("(max-width:600px)").matches;
      ["conditioners", "vrf", "heat-pumps", "fancoils", "ventilation", "microclimate"].forEach(function (n){
        var im = new Image();
        im.src = "/wp-content/uploads/enko/catalog-" + n + "-" + (mob ? "mobile" : "desktop") + ".webp";
      });
    }
    if (document.readyState === "complete") setTimeout(warmSlides, 500);
    else window.addEventListener("load", function (){ setTimeout(warmSlides, 500); });
  }
  ...
  function initAdmin(){ return; /* ENKO: /manager/ керується enko-manager.js */
  ...
  function initMessengerPhotos(){
    var ICON = { tg: "/wp-content/uploads/enko/messengers-tg.webp", vb: "/wp-content/uploads/enko/messengers-viber.webp", wa: "/wp-content/uploads/enko/messengers-whatsapp.webp" };
  ...
  function initAdminTests(){ return; /* ENKO: дубль — initTests у enko-manager.js */
  ...
  /* PDP sticky-бар — підсумок ЗАЯВКИ (вимога замовника 2026-07-13):
     ліворуч одна позиція → її марка/модель (+фото), кілька різних → «Нова заявка»;
     бейдж −N% персональної знижки; праворуч — загальна сума. Порожня заявка →
     бар приховано. Оновлюється при додаванні/зміні к-сті та між вкладками. */
  function initStickyCartTotal(){
    var uahEl = document.getElementById("sb-cart-uah");
    if (!uahEl) return;
    var eurEl = document.getElementById("sb-cart-eur");
    var bar = document.querySelector(".sticky-bar");
    var titleEl = document.getElementById("sb-cart-title");
    var mediaEl = document.getElementById("sb-cart-media");
    var discEl = document.getElementById("sb-discount");
    function update(){
      var c = []; try { c = JSON.parse(localStorage.getItem("enko_cart_v1") || "[]"); } catch (e) {}
      var rate = getRate();
      var t = c.reduce(function (a, i){
      ...
      if (bar) bar.style.display = c.length ? "" : "none";
      if (titleEl && c.length){
        var byId = {}; c.forEach(function (i){ byId[i.id || i.name] = i; });
        var ids = Object.keys(byId);
        if (ids.length === 1){
          var it = byId[ids[0]];
          titleEl.textContent = (it.name || "") + (it.ver ? " · вер. " + it.ver : "");
          if (mediaEl){
            var im = mediaEl.querySelector("img");
            if (im && it.photo) im.src = it.photo;
            mediaEl.style.display = "";
          }
        } else {
          titleEl.textContent = "Нова заявка";
          if (mediaEl) mediaEl.style.display = "none";
        }
      }
      if (discEl){ discEl.textContent = "−" + d + "%"; discEl.hidden = !d; }
    }
    update();
